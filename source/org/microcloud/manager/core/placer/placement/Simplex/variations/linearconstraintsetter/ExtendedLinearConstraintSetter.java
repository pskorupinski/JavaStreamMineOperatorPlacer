...
public class ExtendedLinearConstraintSetter extends VariationImplCore implements LinearContstraintSetter {
	
	WorkersNumberApprox workersNumberApprox;
	RetrievalLimit retrievalLimit;

	public ExtendedLinearConstraintSetter(PlacementProblem placementProblem) {
		...
		this.retrievalLimit = (RetrievalLimit)  Factory.getInstance().newInstance("retrievalLimit",paramTypes,params);
	}
	...
	public LinearConstraintSet create(List<TotalConnectionExecution> totalConnExecList, int... args) {
		...
		/* 
		 * RETRIEVAL LIMITS FOR EVERY SOURCE
		 * 
		 *  "limit of retrieval size for every of the nodes" constraint
		 */
			

		double [] retrSizes = new double[srcHostsNo];
		double [][] hostsForSrcHost = new double[srcHostsNo][totalConnExecList.size()];
		connNum=0;
		for(TotalConnectionExecution conn : totalConnExecList) {	
			/*
			 * 
			 */
			int srcHostNo = conn.getS().getHost().getOrderNumber();
			
			/* if not already defined, define a max retrieval size for this source host */
			if(retrSizes[srcHostNo] == 0.0d) {
				retrSizes[srcHostNo] = retrievalLimit.countFor(conn.getS().getHost());
			}
			
			/*  */
			hostsForSrcHost[srcHostNo][connNum] = 1;	
		...
		for(int i=0; i<destsNo; i++) {
			LinearConstraint linearContraint = 
					new LinearConstraint(hostsForSrcHost[i], Relationship.LEQ, retrSizes[i]);
			org.microcloud.manager.logger.MyLogger.getInstance().log(
					Arrays.toString(linearContraint.getCoefficients().toArray()) + 
							" <== " + 
							linearContraint.getValue());
